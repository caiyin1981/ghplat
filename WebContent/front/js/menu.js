$(function () {
    ...
    $menuLi.hover(function () {
        ...
        if (num != 4) {
            $menu.stop().animate({ backgroundPosition: (100 * num)+10 + "px" + " bottom" }, 100 * len);
        }
        else {
            $menu.stop().animate({ backgroundPosition: (100 * num+10 + 30) + "px" + " bottom" }, 100 * len);
        }
    });
    $li_3.hover(function () {
    ...
    $menu.mouseleave(function () {
        ...
        if (len <= 0) { len = -len; };
        $menu.stop().animate({ backgroundPosition: (100 * current + 10) + "px" + " bottom" }, 100 * len);
    });
